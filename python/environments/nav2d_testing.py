...
class Nav2D(MujocoEnv):
    ...
    def step(self, action):
        ...
        if distance_cond:
            ...
            # rew_head = max((1/(np.pi * (abs_diff + 1e-8))),1)
            rew_head = min(max(1/(np.pi * abs_diff),-1),1)
            rew_head += 0.5 if abs_diff < np.pi/18 else 0
            ...
            #--- reward for closing the distance between the agent and the goal
            rew_dist = self.d_goal_last - d_goal

            #--- penalty for stopping/stalling
            pen_stop = -0.05 if np.abs(self.d_goal_last - d_goal) < 1e-5 else 0.0
            ...
            rew = self.rew_head_scale*rew_head  + self.rew_dist_scale*rew_dist + pen_stop
            ...
            # print(f"ep: {self.episode_counter} | required: {required_heading*(180/np.pi):.2f} | current: {wrapped_theta*(180/np.pi):.2f} | diff: {abs_diff*(180/np.pi):.2f} | rew_head: {rew_head:.2f} | rew_dist: {rew_dist:.5f} | total: {rew:.2f}                                             ", end = "\r")
            # print(f"ep: {self.episode_counter} | action: {action_scale} | required: {required_heading*(180/np.pi):.2f} | current: {wrapped_theta*(180/np.pi):.2f} | diff: {abs_diff*(180/np.pi):.2f} | rew_head: {self.rew_head_scale * rew_head:.2f} | rew_dist: {self.rew_dist_scale * rew_dist:.2f} | total: {rew:.2f}                                             ", end = "\r")
